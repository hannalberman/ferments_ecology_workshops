...
figure_out <- "/Users/hlberman/Library/CloudStorage/GoogleDrive-hlberman@ncsu.edu/My Drive/Dunn_Lab/fermented_foods_public_science/figures_tables/figures"
```
...
```{r, message=FALSE, warning=FALSE, echo=FALSE}
# load data files
metadata_16S0 <- "./FF-16Smetatdata.tsv" %>%
  read_tsv
...
#  CR:  SCOBYPeak is Southern Peak brewery scoby from jar GB2 on Aug 23, Day 43 (last minute addition to sequencing library prep and run). 
```
...
# Experiment Overview
![]('/Users/hlberman/Library/CloudStorage/GoogleDrive-hlberman@ncsu.edu/My Drive/Dunn_Lab/fermented_foods_public_science/figures/Figure2_protocol.png')
...
#save_kable(minLibSizeTable, file.path(figure_out, "minLibSizeTable.png"), zoom=4)
```
...
plot_grid(libSizePlot, bottom_row, ncol = 1, rel_widths = 1, rel_heights = c(1,0.25), labels = c("A", "B"), label_size = 15)
...
## Rarefaction curve
  Rarefied vs. observed ASVs
```{r, message=FALSE, warning=FALSE, echo=FALSE}
S <- map(asvMatrices, specnumber) # observed number of ASVs per sample
raremax <- map(asvMatrices, ~min(rowSums(.x)))

#rarefy samples
#Srare <- map2(asvMatrices, raremax, rarefy)

#create plotting function to pass results to
#plotRarefiedASVs <- function(x,y,z){
#  plot(x, y, main=z, xlab = "Observed no. of ASVs", ylab = "Rarefied no. of ASVs")
#  abline(0,1)
#}
#plot
#pmap(list(S, Srare, names(S)), plotRarefiedASVs)
```
  Rarefaction curve
```{r}
#takes a while, only run when needed
#map2(asvMatrices, names(asvMatrices), ~vegan::rarecurve(.x, step = 20, sample = raremax, col = "blue", cex = 0.6, xlim=c(0,4000))+title(main=.y))

vegan::rarecurve(asvMatrices$Chow, step = 20, sample = raremax, ylab = "ASVs", col = "blue", cex = 0.6, xlim=c(0,4000))
title(main="Chow")

vegan::rarecurve(asvMatrices$Kimchi, step = 20, sample = raremax, ylab = "ASVs", label = FALSE, col = "blue", cex = 0.6, xlim=c(0,4000))
title(main="Kimchi")

vegan::rarecurve(asvMatrices$Kombucha, step = 20, sample = raremax, ylab = "ASVs", col = "blue", cex = 0.6, xlim=c(0,4000))
title(main="Kombucha")

vegan::rarecurve(asvMatrices$`Kombucha (Fungi)`, step = 20, sample = raremax, ylab = "ASVs", col = "blue", cex = 0.6, xlim=c(0,4000))
...
S %>%
  ...
  geom_quasirandom() +
  scale_color_manual(values=c("#000000", "#56B4E9", "#D55E00", "#009E73")) +
  #scale_y_continuous(n.breaks=16, limits = c(0,150)) +
...
# # get top ASVs and percents of ASVs not in top 
# top_ASVs <- ASV_df_prop %>%
#   group_by(Experiment, ASVID, Genus, Species) %>%
#   summarize(`Mean abundance`=mean(Abundance)) %>%
#   group_by(Experiment) %>%
#   top_n(10, `Mean abundance`) %>%
#   select(Experiment, ASVID)
# 
# ASV_df_prop %>%
#   right_join(top_ASVs) %>%
...
bar_plots <- ASV_df_prop %>%
  ...
  map(~ggplot(.x, aes(x=JarID, y=Abundance, fill=Species2)) +
  geom_bar(stat="identity") +
  facet_grid(DayO~Experiment, scales = "free_x") +
  scale_fill_manual(values=c("gray40","#CC79A7", "#0072B2",  "#D55E00",  "#56B4E9", "#009E73",  "#E69F00", "darkslateblue", "#F0E442",  "darkgreen")) +
...
# ASV_df_prop %>%
#   fixTaxaNAs() %>%
#   with_groups(c(SampleID, Experiment, Order, Family, Genus, Species, Species2), summarize, Abundance=sum(Abundance)) %>%
#   with_groups(c(Experiment, Order, Family, Genus, Species, Species2), summarize, `Mean abundance`=mean(Abundance)) %>%
#   group_by(Experiment) %>%
#   top_n(9, `Mean abundance`) %>%
#   mutate(rank=rank(-`Mean abundance`))
```
...
shannon_nPlant_plot <- diversity_df %>%
  ...
  labs(x="No. of plants", y="Shannon index")
...
substrate_alpha_plot <- plot_grid(richness_Plant_plot, shannon_Plant_plot, nrow=2)
substrate_alpha_plot
#ggsave(filename = file.path(figure_out, paste0(Sys.Date(), "_alpha_div_substrate.png")))
```
...
plantNMDSKimchi <- data.scores %>%
  ...
  geom_point() +
  facet_wrap(~Experiment, scales = "free") +
  scale_color_manual(values=c("#0072B2", "#D55E00")) + #"#009E73","#000000", "#CC79A7"))
...
```
  plot
```{r, message=FALSE, warning=FALSE}
legends <- plot_grid(kimchiLegend, teaLegend, ncol=1)
row1 <- plot_grid(plantNMDSKimchi, plantNMDSTea3, plantNMDSTea2, nrow=1)
row2 <- plot_grid(plantNMDSTeaITS, plantNMDSTeaITS2, legends, nrow=1)
row12 <- plot_grid(row1, add_sub(row2, "NMDS1"), ncol=1)
substrate_beta_plot <- plot_grid(add_sub(plot_grid(ggplot+theme_void(), row12, nrow=1, rel_widths = c(0.05,1)), "NMDS2",  angle=90, x=0.05, y=4))
```
...
plot_grid(substrate_alpha_plot, substrate_beta_plot, nrow = 2, rel_heights = c(0.7, 1), labels = c("A", "B"))
...
pH_day_plot
#ggsave(filename = file.path(figure_out, paste0(Sys.Date(), "_pH_time.png")))
```
...
#ggsave(filename = file.path(figure_out, paste0(Sys.Date(), "_alpha_div_time.png")))
```
...
#ggsave(filename = file.path(figure_out, paste0(Sys.Date(), "_beta_div_time.png")))
```
...
### Figure S2: Succession over time
```{r, message=FALSE, warning=FALSE, fig.height=7, fig.width=16.5}
a <- plot_grid(pH_day_plot, succession_alpha_plot, ncol=1, labels = c("A", "B"))
plot_grid(a, succession_beta_plot, labels = c(" ", "C"))
#ggsave(filename = file.path(figure_out, paste(Sys.Date(), "FigureS2_succession_figure.png", sep="_")))
```
...
#### Figure S3: Relative abundances of significant indicator ASVs
...
ASV_df_prop %>%
  ...
  facet_wrap(~Species2,  nrow=3) + 
  theme(axis.text.x = element_text(size=11),
        strip.text.x = element_text(size=9)) +
  labs(x=NULL)
#ggsave(filename = file.path(figure_out, paste(Sys.Date(), "FigureS3_indval_abundances.png", sep="_")))
```
